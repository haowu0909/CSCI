...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  el_blockfoot_t *last_footer =
  PTR_MINUS_BYTES(block, sizeof(el_blockfoot_t)); //get address of footer of last block
  if((void *) last_footer <= (void*) el_ctl.heap_start){
    return NULL;           //given block is higher
  }
  else{
    return el_get_header(last_footer);  //return header of last block
  }
}
...
void el_add_block_front(el_blocklist_t *list, el_blockhead_t *block){
  ...
  block->next->prev = block;  //connection

  list->length++; // increase length
  list->bytes += block->size + EL_BLOCK_OVERHEAD;  // Length is updated to include the new block's size and its overhead.
}
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block){
  ...
  block->next->prev = block->prev;  //unlink

  list->length--; // decrease size
  list->bytes -= (block->size + EL_BLOCK_OVERHEAD);  // update length
...
el_blockhead_t *el_find_first_avail(size_t size){
  el_blockhead_t *available = el_ctl.avail->beg->next;

  while(available != el_ctl.avail->end){
    if(available == NULL){
      return NULL;
  }
    if (available->size >= size+EL_BLOCK_OVERHEAD){ //block size of at least (size+EL_BLOCK_OVERHEAD).
      return available;
  }
    available = available -> next;  //search for next block
}
  return NULL;
}
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
   //new size  = 60, size of block before split is 300 + overhead = 340
  size_t after_size = block->size - new_size;  //300 + 40-100=240

  el_blockfoot_t *before_footer = el_get_footer(block);  //change footer
  before_footer->size = after_size - EL_BLOCK_OVERHEAD; //240-40 = 200
  block->size = new_size;// size in header is 60

  el_blockfoot_t *new_foot = el_get_footer(block); //new footer of used block
  new_foot->size = new_size;

  el_blockhead_t *new_head = el_block_above(block);//new header of used block
  new_head->size = after_size - EL_BLOCK_OVERHEAD;  //240 - 40
  new_head -> state = EL_AVAILABLE;  //set state to available

  return new_head;

  if (block->size < new_size + EL_BLOCK_OVERHEAD){  // (at least new_size + EL_BLOCK_OVERHEAD for the new header/footer)
    return NULL;
  }
...
void *el_malloc(size_t nbytes){
  el_blockhead_t *block = el_find_first_avail(nbytes);//find first available head
  if (block == NULL){
      return NULL;
  }
  el_remove_block(el_ctl.avail,block); //remove from available lists
  block -> state = EL_USED;            //set state to used

  el_blockhead_t *new_avail_head = el_split_block(block, nbytes);

  el_add_block_front(el_ctl.used, block);
  el_add_block_front(el_ctl.avail, new_avail_head);

  void *usable_space = PTR_PLUS_BYTES(block, sizeof(el_blockhead_t));// Return pointer to a block of memory
  return usable_space;
...
void el_merge_block_with_above(el_blockhead_t *lower){
  ...
  el_blockhead_t *higher = el_block_above(lower);  //create higher if lower is available
  if(higher == NULL || higher->state != EL_AVAILABLE){
  		return;
  	}
  el_remove_block(el_ctl.avail, higher);  // remove higher
  el_remove_block(el_ctl.avail, lower); // remove lower

  size_t new_size = EL_BLOCK_OVERHEAD + higher->size + lower->size; //40+90+60
  lower->size = new_size;  // update the size in head of lower
  el_blockfoot_t *foot = el_get_footer(lower);
  foot->size = new_size;   // update the size in foot of higher

  el_add_block_front(el_ctl.avail, lower); //add loewer to available list
...
void el_free(void *ptr){
  el_blockhead_t *free_block = PTR_MINUS_BYTES(ptr, sizeof(el_blockhead_t));
  el_blockhead_t *lower = el_block_below(free_block);

  el_remove_block(el_ctl.used, free_block);
  free_block->state = EL_AVAILABLE;      //update state
  el_add_block_front(el_ctl.avail, free_block);

  el_merge_block_with_above(free_block);   // merge
