#!/bin/bash
#
# Mon 09 Dec 2019 09:44:59 PM CST : bug fix to 'unset BASH_ENV' which
# removes output from CSE Labs module systems code

# fixes a bug with environment variables introduced by the CS labs
# module system: BASH_ENV can be a script that is run as a init file
# for non-interactive shells; it is set by the module syst
unset BASH_ENV

if [ ! -d "test-input" ]; then
    printf "No 'test-input' directory found, cannot proceed with tests\n"
    exit 1
fi

generate=0
if [ "$1" == "-gen" ]; then
    generate=1
    shift
fi

run_norm=1                                 # run normal tests
run_valg=1                                 # run valgrind tests

VALGRIND="valgrind --leak-check=full --show-leak-kinds=all"
DIFF="diff -bBy"                            # -b ignore whitespace, -B ignore blank lines, -y do side-by-side comparison

mkdir -p test-data

INPUT=test-data/input.tmp                   # name for expected output file
EXPECT=test-data/expect.tmp                 # name for expected output file
ACTUAL=test-data/actual.tmp                 # name for actual output file
DIFFOUT=test-data/diff.tmp                  # name for diff output file
VALGOUT=test-data/valgrind.tmp              # name for valgrind output file

function major_sep(){
    printf '%s\n' '====================================='
}
function minor_sep(){
    printf '%s\n' '-------------------------------------'
}

DATAFILE="test_patchsym_data.sh"
printf "Loading tests from $DATAFILE... "
source "$DATAFILE"
printf "%d tests loaded\n" "$T"

NTESTS=$T
VTESTS=$T
NPASS=0
NVALG=0

all_tests=$(seq $NTESTS)


# Check whether a single test is being run
single_test=$1
if ((single_test > 0 && single_test <= NTESTS)); then
    printf "Running single TEST %d\n" "$single_test"
    all_tests=$single_test
    NTESTS=1
    VTESTS=1
else
    printf "Running %d tests\n" "$NTESTS"
fi

# printf "tests: %s\n" "$all_tests"
printf "\n"

# Run normal tests: capture output and check against expected
if [ "$run_norm" = "1" ]; then
    printf "RUNNING NORMAL TESTS\n"
    for i in $all_tests; do
        printf "TEST %2d %-35s : " "$i" "${tnames[i]}"
        FAIL="0"
        
        # Run the test

        # run program with given input, -x prints commands as they are run preceded by PS4
        
        PS4='> ' bash -x -c "${cmd[i]}" >& $ACTUAL
        sed -i 's/^+/>/g' $ACTUAL     # in case PS4 is not respected, replace leading '+' with '>'
        # generate expected output
        printf "%s\n" "${output[i]}" > $EXPECT

        # Check for output differences, print side-by-side diff if problems
        if ! $DIFF $EXPECT $ACTUAL > $DIFFOUT
        then
            printf "FAIL: Output Incorrect\n"
            minor_sep
            # printf "INPUT:\n%s\n" "${cmd[i]}"
            printf "OUTPUT: EXPECT   vs   ACTUAL\n"
            cat $DIFFOUT
            if [ "$generate" == "1" ]; then
                printf "====FULL ACTUAL====\n"
                cat $ACTUAL
            fi
            minor_sep
            FAIL="1"
        fi

        if (( FAIL == 0 )); then
            printf "OK\n"
            ((NPASS++))
        fi            
    done
    printf "Finished:\n"
    printf "%2d / %2d Normal correct\n" "$NPASS" "$NTESTS"
    printf "\n"
fi

# ================================================================================

# Run valgrind tests: check only for problems identified by valgrind
if [ "$run_valg" = "1" ]; then
    printf "RUNNING VALGRIND TESTS\n"

    for i in $all_tests; do
        printf "TEST %2d %-35s : " "$i" "${tnames[i]}"
        
        # run code through valgrind
        input=$(echo "${cmd[i]}" | sed "s/\.\/patchsym/$VALGRIND \.\/patchsym/g")
        PS4='> ' bash -x -c "$input" >& $VALGOUT
        sed -i 's/^+/>/g' $ACTUAL     # in case PS4 is not respected, replace leading '+' with '>'

        # Check various outputs from valgrind
        if ! grep -q 'ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)' $VALGOUT ||
           ! grep -q 'in use at exit: 0 bytes in 0 blocks'  $VALGOUT ||
             grep -q 'definitely lost: 0 bytes in 0 blocks' $VALGOUT;
        then
            printf "FAIL: Valgrind detected problems\n"
            minor_sep
            cat $VALGOUT
            minor_sep
        else
            printf "Valgrind OK\n"
            ((NVALG++))
        fi
    done
    printf "Finished:\n"
    printf "%2d / %2d Valgrind correct\n" "$NVALG" "$VTESTS"
    printf "\n"
fi


major_sep
printf "OVERALL:\n"
printf "%2d / %2d Normal correct\n" "$NPASS" "$NTESTS"
printf "%2d / %2d Valgrind correct\n" "$NVALG" "$VTESTS"

TOTPASS=$((NPASS+NVALG))
NTOTAL=$((NTESTS+VTESTS))

printf "RESULTS: %d / %d points for normal/valgrind tests\n" "$TOTPASS" "$NTOTAL"
